...
## Core loop (daily)
1. App fetches new reviews (last 7 days) for all the developer's apps via Google Play Developer API using the developer's own Google sign-in (OAuth, scope androidpublisher).
...
## Architecture
- Android app (Kotlin, Jetpack Compose), Room DB for review history, WorkManager daily sync.
- Google Play Developer API v3 via REST (OkHttp) with Google Sign-In (OAuth) — no service accounts.
